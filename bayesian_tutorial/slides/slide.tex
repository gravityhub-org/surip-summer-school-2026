...
\begin{frame}{Macau story: Simplified}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.6\textwidth}\large
			\cc{Suppose}
			\ccc{We toss the coin 10 times and get 8 tails}
...
\begin{frame}{Macau story}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.6\textwidth}\large
			\cc{Suppose}
			\ccc{The probability that a coin returns tail is 0.5}
			\ccc{We toss the coin 10 times and get 8 tails}
			\cc{What is the probability of getting 8 tails in 10 tosses?}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Macau story: Simplified}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.6\textwidth}\large
			...
			\cc{What is the probability of getting 8 tails in 10 tosses?}
			\ccc{Answer:}
			$$P(T=8) = \binom{N}{T}\cdot 0.5^T \cdot (1-0.5)^{N-T} = 45/1024 \quad (N{=}10,\; T{=}8)$$
...
\begin{frame}{Introducing competing hypotheses}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}\large
			...
			\cc{Probability of observing the data ($T$ tails in $N$ tosses) under each hypothesis:}
			$$P(T|H_\text{fair},N) = \binom{N}{T} \cdot (0.5)^T \cdot (1-0.5)^{N-T}$$
			$$P(T|H_\text{rigged},q,N) = \binom{N}{T} \cdot q^T \cdot (1-q)^{N-T}$$
			$T$ is the number of tails; coin bias $q$ is the tail probability under $H_\text{rigged}$.\\
			\cc{Under which hypothesis is the data more likely?}
			\ccc{Uniform prior on $q$: $P(q|H_\text{rigged}) = 1$ on $[0,1]$}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Bayes factor}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}\large
			...
			\cc{Uniform prior on $q$: $P(q|H_\text{rigged}) = 1$ on $[0,1]$}
...
\begin{frame}{Surely it's rigged now?}
	...
	\begin{columns}[T]
		...
		\hfill
		\begin{column}{0.7\textwidth}\large
			\cc{You have seven datasets $\vec d$}
			\ccc{Let us assume that the datasets are independent}
			$$P(\vec d|H_\text{rigged},N) = \prod_{i=1}^{7} P(d_i|H_\text{rigged},N)$$
			$$P(\vec d|H_\text{fair},N) = \prod_{i=1}^{7} P(d_i|H_\text{fair},N)$$
			$$B^r_f \approx (2.068\cdots)^7 \approx 150$$
...
\begin{frame}{Surely it's rigged now?}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}\large
			\cc{You repeat the experiment 7 times, each experiment gives 8 tails in 10 tosses.}
			\ccc{Is the coin rigged?}
			\ccc{A rigged coin is around 150 times more likely to generate the data than a fair coin.}
...
\begin{frame}{Posterior odds}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}\large
			...
			$$= \frac{P(\vec d|H_\text{rigged},{\color{blue}I})}{P(\vec d|H_\text{fair},{\color{blue}I})} \cdot \frac{P(H_\text{rigged}|{\color{blue}I})}{P(H_\text{fair}|{\color{blue}I})} = 150 \cdot \frac{P(H_\text{rigged}|{\color{blue}I})}{P(H_\text{fair}|{\color{blue}I})}$$
			\cc{Only $<$ 1 in 10,000 casinos are rigged}
			\ccc{Macau Casinos are heavily regulated.}
			\ccc{Prior odds of rigged vs fair: $1/10,000$}
			$$O^r_f = 150 \cdot \frac{1}{10,000} = 0.015\approx1/67$$
...
\begin{frame}{Home casino}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}\large
			...
			\cc{You get 8 tails in 10 tosses, 7 times in a row.}
			\ccc{Is the coin rigged now?}
			$$O^r_f = \frac{P(H_\text{rigged}|\vec d, {\color{blue}I})}{P(H_\text{fair}|\vec d,{\color{blue}I})}$$
			$$= \frac{P(\vec d|H_\text{rigged},{\color{blue}I})}{P(\vec d|H_\text{fair},{\color{blue}I})} \cdot \frac{P(H_\text{rigged}|{\color{blue}I})}{P(H_\text{fair}|{\color{blue}I})} = 150 \cdot \frac{P(H_\text{rigged}|{\color{blue}I})}{P(H_\text{fair}|{\color{blue}I})}$$
			\cc{Prior odds of rigged vs fair: $1/1$}
			$$O^r_f = 150 \cdot \frac{1}{1} = 150$$
...
\begin{frame}{What can we tell about the coin?}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}\large
			...
			\ccc{Uniform prior: $P(q|H_\text{rigged}) = 1$ on $[0,1]$}
			$$= \frac{\prod_{i=1}^{7} P(d_i|H_\text{rigged},q) \cdot P(q|H_\text{rigged})}{P(\vec d|H_\text{rigged})}$$
			$$= \frac{\prod_{i=1}^{7} \binom{N}{T} \cdot q^T \cdot (1-q)^{N-T} \cdot P(q|H_\text{rigged})}{P(\vec d|H_\text{rigged})}$$
			\cc{The coin is probably rigged and returns tails around 70-90 percent of the time.}
		\end{column}
	\end{columns}
\end{frame}
